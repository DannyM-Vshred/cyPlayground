...
```
yarn cypress run --project ./src/Ecommerce/StoreFront --config-file le.sn.config.ts --spec "src/Ecommerce/StoreFront/features/adminTool/01_create_and_delete_user.feature"  --env CUSTOM_TEST_DATA_JSON=01_create_and_delete_user -b chrome --headed
```
...
The Ecommerce test project supports running tests with a different geolocation emulated in the browser. To achieve this, set the `GEOLOCATION` environment variable in your `cypress.env.json` local file to one of the values from the `common-web-test-automation-framework/src/Ecommerce/StoreFront/cypress/support/testData/geolocations.ts` file. For example, "sydney".
...
The Ecommerce test project supports running tests in CircleCi on a different URL than the default baseUrl specified in the `le.sn.config.ts` file. To do so, just add a new `string` parameter called `url` to the list of available parameters in the Trigger Pipeline popup of CircleCi.
