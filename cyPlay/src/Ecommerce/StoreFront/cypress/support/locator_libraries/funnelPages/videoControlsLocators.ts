const videoControlsLocators = {
    ...
    videoId: () => cy.get('#vidalytics_video'),
    startStopControler: () => cy.get('.video-container'),
    pausedVideo: () => cy.get('.jw-state-paused'),
    playingVideo: () => cy.get('.CallToActionsManager__wrapper--fullscreen'),
    vidalyticsId: (vidId: string) => cy.get(`#vidalytics_embed_${vidId}`),
    playingVfullScreenideo: () => cy.get('.ContextMenu__wrapper'),
    claimLimitedTimeButton: () => cy.findByText('Claim Limited Time Offer Now'),
    progressionPercent: () =>
        cy.get('.ProgressLine__track').then(($el) => {
