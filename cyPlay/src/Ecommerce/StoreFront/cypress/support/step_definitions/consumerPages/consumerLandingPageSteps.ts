...
import { validatePageSection } from 'Ecommerce/StoreFront/cypress/commands';
import {
...
Given('The user starts on full url {string} page', () => {
    cy.get('@staticTestData').then((staticTestData: any) => {
...
Given(
    'The user starts on full survey-ga VSL url {string} page',
    (urlKey: string) => {
        cy.get('@staticTestData').then((staticTestData: any) => {
            const { fullurls } = staticTestData.LandingPage;
            ...
            const urlMap: Record<string, number> = {
                ...
                'fitness-male-cb': 10,
                'fitness-male-fl': 11,
                'fitness-female-gt': 12,
                'fitness-female-sf': 13,
            };

            const baseUrl = fullurls[urlMap[urlKey]];

            if (!baseUrl) {
                throw new Error(`Invalid urlKey: ${urlKey}`);
            }

            const url = new URL(baseUrl);

            url.searchParams.set('qa_test', 'VSQAT');

            if (
                baseUrl.includes('le.vshred.com') ||
                baseUrl.includes('le-uat.vshred.com')
            ) {
                url.searchParams.set('force_gateway', 'Braintree');
            }
    ...
    () => {
        ...
        cy.get('@staticTestData').then((staticTestData: any) => {
            ...
            cy.url().then((checkoutPageUrl: any) => {
                ...
                if (isUatProdServer) {
                    cy.visit(
                        `${checkoutPageUrl}&force_gateway=Braintree&qa_test=VSQAT`
                    );
                }
                if (isProdServer) {
                    cy.visit(
...
When(
    ...
    (upsellOffer: string) => {
        ...
        // eslint-disable-next-line cypress/no-unnecessary-waiting
        cy.wait(8000); // expected delay creating vsu subscription
    }
);
...
Then('The User Declines the Burn 1B Upsell Funnel Offer', () => {
    cy.skipVideo();
    consumerLandingPageLocators.productPage
        .declineBurn1B()
        .scrollIntoView()
...
Then('The User Declines the recipe guide Upsell Funnel Offer', () => {
    cy.skipVideo();
    consumerLandingPageLocators.productPage
        .declineRecipeGuide()
        .scrollIntoView()
...
Then('The User Declines the VSU Upsell Funnel Offer', () => {
    cy.skipVideo();
    consumerLandingPageLocators.productPage
        .declineVsu()
        .scrollIntoView()
...
When('The user verifies IP and Location', () => {
...
When('The user navigates to V Shred Survey-ga with show page', () => {
    cy.clearAllSessionData();
    // cy.get('@staticTestData').then((staticTestData: any) => {
    const leVshredUrl = 'https://le.vshred.com/sp/survey/survey-ga?show=11149';
    const isProdServer =
        leVshredUrl.includes(`https://le.vshred.com`) ||
        leVshredUrl.includes(`https://vshred.com`) ||
        leVshredUrl.includes(`https://le.vshredmd.com`) ||
    ...
    if (isProdServer) {
        // const { surveyUrl } = staticTestData.surveyLandingPage;
        cy.visit(`${leVshredUrl}&qa_test=VSQAT`);
    } else {
        // const { surveyUrl } = staticTestData.surveyLandingPage;
        cy.visit(`${leVshredUrl}`);
    }
    consumerLandingPageLocators.loader().should('not.exist');
    cy.skipVideo();
    // });
...
When('The user navigates to V Shred Survey-ga pre aug 4th page', () => {
    ...
    // cy.get('@staticTestData').then((staticTestData: any) => {
    const leVshredUrl =
        'https://le-testing-14.vshred.com/sp/survey-0804/survey-ga';
    const isProdServer =
        leVshredUrl.includes(`https://le.vshred.com`) ||
        leVshredUrl.includes(`https://vshred.com`) ||
        leVshredUrl.includes(`https://le.vshredmd.com`) ||
...
Then('Connects to LE Blue Production Server', () => {
    const targetUrl =
        'https://le.vshred.com/fast-checkout?funnel=btq-fb-f&offer_skus=bun_59eb9f8317f1b&qa_test=VSQAT'; // Production URLs
    // https://le.vshred.com/
    // 1. Intercept the request to the target URL and add the header
    cy.intercept('GET', targetUrl, (req) => {
        req.headers['x-server-name'] = 'blue';
        req.headers['x-canary'] = 'blue';
    }).as('pageLoad'); // Give the intercept a unique alias

    // 2. Visit the page, which will now be intercepted
    cy.visit(targetUrl);

    // 3. Wait for the intercepted request to complete
    cy.wait('@pageLoad').then((interception) => {
        // Log the request headers from the interception object for verification
        cy.log(
            'Request Headers:',
            JSON.stringify(interception.request.headers)
        );

        // 4. Assert that the header was added to the request
        expect(interception.request.headers).to.have.property('x-server-name');
        expect(interception.request.headers['x-server-name']).to.eq('blue');

        // You can also assert on the response headers
        // expect(interception.response.headers).to.have.property('x-server-name');
        expect(interception.response.headers['x-server-name']).to.match(
            /^le-production-blue/
        );
    });
});
...
Then('Connects to SN Blue Production Server', () => {
    const targetUrl = 'https://sculptnation.com/';
    cy.intercept('GET', targetUrl, (req) => {
        req.headers['x-server-name'] = 'blue';
        req.headers['x-canary'] = 'blue';
    }).as('pageLoad');
...
Then('Connects to LE Orange Production Server', () => {
    const targetUrl = 'https://le.vshred.com/';
    cy.intercept('GET', targetUrl, (req) => {
        req.headers['x-server-name'] = 'orange';
        req.headers['x-canary'] = 'orange';
    }).as('pageLoad');
    ...
    cy.wait('@pageLoad').then((interception) => {
        ...
        expect(interception.request.headers).to.have.property('x-server-name');
        expect(interception.request.headers['x-server-name']).to.eq('orange');
        expect(interception.response.headers['x-server-name']).to.match(
...
Then('Connects to LE Red Production Server', () => {
    const targetUrl = 'https://le.vshred.com/';
    cy.intercept('GET', targetUrl, (req) => {
        req.headers['x-server-name'] = 'red';
        req.headers['x-canary'] = 'red';
    }).as('pageLoad');
    ...
    cy.wait('@pageLoad').then((interception) => {
        ...
        expect(interception.request.headers).to.have.property('x-server-name');
        expect(interception.request.headers['x-server-name']).to.eq('red');
        expect(interception.response.headers['x-server-name']).to.match(
            /^le-production-red/
        );
    });
...
Then('Connects to SN Red Production Server', () => {
    const targetUrl = 'https://sculptnation.com/';
    cy.intercept('GET', targetUrl, (req) => {
        req.headers['x-server-name'] = 'red';
        req.headers['x-canary'] = 'red';
